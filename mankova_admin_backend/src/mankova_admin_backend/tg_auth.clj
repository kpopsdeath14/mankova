(ns mankova-admin-backend.tg-auth
  (:require
...
(defn verify-telegram-hash 
  ...
  (let [decoded (decode-url-utf8 data)
    ...
    (println "data-check-string: " data-check-string)
    (println "computed-hash: " computed-hash)
    (println "provided-hash: " provided-hash)
    (println (= computed-hash provided-hash))
...
(defn wrap-telegram-auth [handler bot-token] 
  ...
  (fn [request] 
    (let [user-data-hash (or (get-in request [:headers "x-telegram-hash"])
      ...
      (println "user-data-hash = " user-data-hash) 
