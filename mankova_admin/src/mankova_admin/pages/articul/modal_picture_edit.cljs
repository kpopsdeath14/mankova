(ns mankova-admin.pages.articul.modal-picture-edit
  (:require
   ...
   [mankova-admin.events.articul-img-delete :refer [images_del]]
   ))
...
(defn image_grid [imgs]
  (let [Row antd/Row
        ...
        images_edit (reagent/cursor app-state [:images_edit])
        ...
        delete-image (fn [img]
                       ...
                       (let [new-images (vec (remove #{img} @images_edit))] 
                         (swap! app-state assoc-in [:images_edit] new-images)
                         )
                       )
        
        move-image (fn [img direction]
                     (let [current-images (:images_edit @app-state)
     ...
     [:> Col {:span 8
      ...
      [:> Button {:style {:background "#D3EAFF"
                  ...
                  :onClick (fn [] 
                             (if @edit-mode?
                               (do
                                 (images_upd (:product_id @articul_changes) (:images_edit @app-state))
                                 (images_del (vec_difference (:images (:articul_changes @app-state)) (:images_edit @app-state)))
                                 )
                          ...
                          (let [status (.-status info.file)
                            ...
                            (when all-finished?
                              (let [successful-files (->> file-list
                                ...
                                (swap! app-state assoc-in [:images_edit] (vec (concat (get-in @app-state [:images_edit]) (mapv #(:filename (js->clj (.-response %) :keywordize-keys true)) file-list))))
    ...
    (fn []
      [:> Modal
       {:visible @visible?
        ...
        :onCancel (fn []
                    (swap! app-state assoc :product_pictures_edit_mode false)
                    (images_del (vec_difference @images_edit (:images @articul_changes)))
                    (swap! app-state assoc :show_modal_picture_edit? false)
                    (swap! app-state assoc :images_edit [])
                    )
        }
